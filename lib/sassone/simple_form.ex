defmodule Sassone.SimpleForm do
  @moduledoc ~S"""
  ...
  * `:cdata_as_characters` - `true` to return CData as characters, `false` to wrap CData as `{:cdata, data}`. Defaults to `true`.
  ...
  @type content() :: [String.t() | {:cdata, String.t()} | Sassone.SimpleForm.t()]

  @type t() :: {tag_name(), attributes(), content()}

  @spec parse_string(data :: binary(), options :: Keyword.t()) ::
          {:ok, Sassone.SimpleForm.t()} | {:error, exception :: Sassone.ParseError.t()}

  def parse_string(data, options \\ []) when is_binary(data) do
    case Sassone.parse_string(data, __MODULE__.Handler, [], options) do
      {:ok, document} ->
        {:ok, document}
