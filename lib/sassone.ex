defmodule Sassone do
  @moduledoc ~S"""
  Sassone is an XML SAX parser and encoder.

  Sassone provides functions to parse XML file in both binary and streaming way in compliant
  with [Extensible Markup Language (XML) 1.0 (Fifth Edition)](https://www.w3.org/TR/xml/).

  Sassone also offers DSL and API to transform structs to and from XML documents.
  ...
  * `:end_document` - when the root element is closed.
  ...
  Sassone defaults the encoding to UTF-8.
  ...
  Sassone does not expand external entity references, but provides an option to specify how they should be handled.
  ...
  Sassone does not support parsing DTD (Doctype Definition) and XSD schemas. When encountering DTD, the parser simply
  skips that.
  ...
  * `:cdata_as_characters` - `true` to emit CData events as `:characters`. Defaults to `true`.
  ...
  alias Sassone.{Handler, ParseError, Parser, Prolog, XML}
  ...
  During parsing, the actual memory used by Sassone might be higher than the number configured for each chunk, since
  Sassone holds in memory some parsed parts of the original binary to leverage Erlang sub-binary extracting. Anyway,
  Sassone tries to free those up when it makes sense.
  ...
  * `:character_data_max_length` - tells the parser to emit the `:characters` event when its length exceeds the specified
  ...
  def encode_to_iodata!(root, prolog \\ nil), do: encode_to_iodata(root, prolog)

  defp encode_to_iodata(root, prolog) do
  ...
  defp attributes([{nil, name, value} | attributes]),
    do: [?\s, name, ?=, ?", escape(value, 0, value), ?" | attributes(attributes)]

  defp attributes([{namespace, name, value} | attributes]),
    do: [?\s, namespace, ?:, name, ?=, ?", escape(value, 0, value), ?" | attributes(attributes)]
