defmodule Sassone.Parser.PrologTest do
  ...
  describe "malformed XML prolog" do
    ...
    test "invalid ending quote in encoding declaration" do
      ...
      assert Exception.message(error) ==
               "unexpected byte \"\\\"\", expected token: :encoding_name"
    ...
    test "invalid encoding value" do
      ...
      assert Exception.message(error) ==
               "unexpected encoding declaration \"abc\", only UTF-8 is supported"
    end
  ...
  describe "misc after XML declaration" do
    test "process instructions" do
      ...
      assert Exception.message(error) ==
               "unexpected byte \"?\", expected token: :processing_instruction"
    end
  ...
  defp prolog_version do
    gen all version <- constant("1.0"),
      ...
      binary =
        IO.iodata_to_binary(["version", equal_sign, wrapping_quote, version, wrapping_quote])
  ...
  defp prolog_encoding do
    gen all encoding <- one_of([constant("utf-8"), constant(nil)]),
      ...
      if encoding do
        binary =
          IO.iodata_to_binary(["encoding", equal_sign, wrapping_quote, encoding, wrapping_quote])
  ...
  defp prolog_standalone do
    gen all standalone <-
              one_of([constant({true, "yes"}), constant({false, "no"}), constant(nil)]),
            wrapping_quote <- xml_quote(),
            equal_sign <- xml_equal_sign() do
      case standalone do
        nil ->
